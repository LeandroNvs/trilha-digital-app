...
// =================================================================
// MUDANÇA: Componente Sumário e Submissão
// =================================================================
function SumarioDecisoes({ 
    simulacao, 
    estadoRodada, 
    decisoes, 
    decisaoRef, 
    ...
    custoUnitarioProjetadoS1,
    custoUnitarioProjetadoS2
}) {
    ...
    const { 
        ...
        const totalInvestOrg = (Number(decisoes.Invest_Organiz_Capacitacao) || 0) + 
    ...
    // Cálculo da Projeção de Vendas (Usa pNumTotal)
    const { receitaProjetada, caixaProjetadoPosVenda, precoMedioPonderado } = useMemo(() => {
        const unidadesVender = pNumTotal * (percentualVendasEstimado / 100);
    ...
    }, [pNumTotal, percentualVendasEstimado, decisoes.Preco_Segmento_1, decisoes.Preco_Segmento_2, caixaProjetadoPosProducao]);


    // =================================================================
    // MUDANÇA: Validação de todas as chaves
    // =================================================================
    ...
    const handleSubmeterClick = () => { /* ... */ setFeedback(''); if (!todasDecisoesPreenchidas) { setFeedback(`Erro: Salve TODAS as abas (1 a 6) antes de submeter.`); return; } setShowConfirm(true); }; 
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg mt-6 space-y-6 animate-fade-in">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div className="bg-gray-700 p-3 rounded-lg"><span className="block text-gray-400">(-) Invest. Organizacional</span><span className="text-red-400 font-semibold">{formatBRLDisplay((Number(decisoes.Invest_Organiz_Capacitacao) || 0) + (Number(decisoes.Invest_Organiz_Mkt_Institucional) || 0) + (Number(decisoes.Invest_Organiz_ESG) || 0))}</span></div>
                 ...
                 {/* MUDANÇA: Label do custo de produção */}
                 <div className="bg-gray-700 p-3 rounded-lg text-sm"><span className="block text-gray-400">(-) Custo Produção Total ({pNumTotal.toLocaleString('pt-BR')} unid.)</span><span className="text-red-400 font-semibold">{formatBRLDisplay(custoTotalProducaoProjetado)}</span></div>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-700">
                ...
                <div className="space-y-2">
                    ...
                    <input type="range" id="vendasSlider" min="0" max="100" step="5" value={percentualVendasEstimado} onChange={(e) => setPercentualVendasEstimado(Number(e.target.value))} disabled={isSubmetido || pNumTotal === 0} className="w-full h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer range-lg accent-cyan-500" />
                </div>
            ...
            <div className="text-right mt-6 pt-6 border-t border-gray-700"> <button onClick={handleSubmeterClick} className={`font-bold py-3 px-8 rounded-lg transition-colors text-lg ${ isSubmetido ? 'bg-gray-600 text-gray-400 cursor-not-allowed' : !todasDecisoesPreenchidas ? 'bg-yellow-600 text-white cursor-not-allowed' : 'bg-green-500 hover:bg-green-600 text-white' }`} disabled={loading || isSubmetido || (!isSubmetido && !todasDecisoesPreenchidas)} title={!isSubmetido && !todasDecisoesPreenchidas ? "Salve TODAS as abas (1 a 6) primeiro" : (isSubmetido ? "Rodada Submetida" : "Submeter decisões")}> {isSubmetido ? 'Rodada Submetida' : (loading ? 'Enviando...' : 'Submeter Decisões da Rodada')} </button> </div>
            {/* Modal Confirmação */}
