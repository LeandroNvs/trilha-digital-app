...
// Corrigindo os caminhos de importação para os módulos necessários
...
function PaginaDisciplinas() {
    ...
    const limparForm = () => {
        setNome(''); setSigla(''); setCursoId(''); setEditando(null);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const disciplinasCollection = collection(db, collectionPath);

        if (editando) {
            const docRef = doc(db, collectionPath, editando.id);
            await updateDoc(docRef, dados);
        } else {
            await addDoc(disciplinasCollection, { ...dados, criadaEm: serverTimestamp() });
        }
        limparForm();
    };

    const handleEdit = (d) => {
        setEditando(d); setNome(d.nome); setSigla(d.sigla); setCursoId(d.cursoId);
    };
    ...
    return (
        <div className="bg-gray-800 shadow-lg rounded-xl p-8 animate-fade-in">
            <h2 className="text-2xl font-bold text-cyan-400 mb-6">Gerenciar Disciplinas</h2>
            <form onSubmit={handleSubmit} className="bg-gray-700 p-6 rounded-lg mb-8 space-y-4">
                <h3 className="text-xl font-semibold">{editando ? 'Editando' : 'Nova'} Disciplina</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" value={nome} onChange={e => setNome(e.target.value)} placeholder="Nome da Disciplina" className="bg-gray-600 rounded-lg p-3" required />
            ...
            <div className="space-y-4">
                {disciplinas.map(d => (
                    <div key={d.id} className="flex items-center justify-between bg-gray-700 p-4 rounded-lg">
                        <div>
                            <p className="font-bold">{d.nome} ({d.sigla})</p>
                            <p className="text-sm text-gray-400">{cursosMapa[d.cursoId]?.sigla} - {cursosMapa[d.cursoId]?.siglaInstituicao}</p>
                        </div>
                        <div className="flex gap-4">
                            <button onClick={() => handleEdit(d)} className="text-yellow-400">Editar</button>
                            <button onClick={() => setItemParaExcluir(d)} className="text-red-500">Excluir</button>
                        </div>
                    </div>
                ))}
            </div>
